	...
	*/
	function ThemeWidgetFacebookFeed() {
		$widget_opts = array(
			...
			'description' => __( 'Displays the Facebook Feed from a user or page.','gymboom' ) // description shown in the widget list
		);
		...
		$this->WP_Widget('theme-widget-facebook-feed', __('[GYMBOOM] Facebook Feed','gymboom'), $widget_opts, $control_ops);
		$this->custom_fields = array(
			array(
				'name'=>'title',
				'type'=>'text',
				'title'=>'Title', 
				'default'=>__('Facebook Feed','gymboom')
			),
			array(
				'name'=>'facebook_id',
				'type'=>'text',
								...
								if ($s['type'] == 'photo'){
									...
									<li>
										<span class="tweet_text">
											<?php echo makeClickableLinks(js_char_shortalize($message)); ?><?php echo '<br /><strong><a href="'.$s['link'].'" target="_blank"><img src="'.$s['icon'].'" /> View photo</a></strong>'; ?>
										</span>
										<span class="tweet_time"><?php echo getRelativeTime($s['created_time'],true); ?></span>
									</li>
									<?php
