...
function widget_field_set($obj, $instance, $fieldname, $fieldtitle, $field_array) {
	...
	<p>
		<label for="<?php echo $obj->get_field_id($fieldname); ?>"><?php echo $fieldtitle; ?>:</label><br />
		<?php foreach ($field_array['options'] as $val => $name) : ?>
			<span style="padding-top:3px; display:block;"><input type="checkbox" name="<?php echo $obj->get_field_name($fieldname); ?>[]" value="<?php echo $val; ?>" <?php echo (is_array($value) && !(in_array($val, $value) === FALSE)) ? 'checked="checked"' : ''; ?>>&nbsp;<?php echo $name; ?></span>
		<?php endforeach; ?>
	</p>
	<?php
}
