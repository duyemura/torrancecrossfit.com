...
function page_array(){
	...
	$pages[''] = __('Select a page...','gymboom');
...
function post_array($type = 'post'){
	...
	if ($type_name == 'custom'){ $posts[''] = 'No slider'; } else { $posts[''] = __('Select','gymboom').' '.$a_text.' '.$type_name.$post_text.'...'; }
	
	foreach ($posts_obj as $post) {
		if ($type_name == 'custom'){
			$posts['GYMBOOM---'.$post->ID] = $post->post_title;
		} else {
