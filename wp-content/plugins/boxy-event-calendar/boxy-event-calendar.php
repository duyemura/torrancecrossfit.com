<?php
/*
Plugin Name: Boxy Event Calendar
Plugin URI: http://boxystudio.com
...
/*  This is a modified version of the Ajax Event Calendar, Copyright 2011 Eran Miller
...
new WPUpdatesPluginUpdater_842( 'http://wp-updates.com/api/2/plugin', plugin_basename(__FILE__));
...
if (version_compare(PHP_VERSION, '5', '<')) {
	...
	$out .= sprintf("<p><strong>Your PHP version is '%s'.<br>The Boxy Event Calendar WordPress plugin requires PHP 5 or higher.</strong></p><p>Ask your web host how to enable PHP 5 on your site.</p>", PHP_VERSION);
	$out .= "</div>";
	print $out;
}

define('AEC_MENU_POSITION', 30);
define('AEC_VERSION', '2.0.0');
...
define('AEC_HOMEPAGE', 'http://boxystudio.com');
...
if (!class_exists('ajax_event_calendar')) {
	class ajax_event_calendar {
		...
		public static function install(){
			...
			if ($wpdb->get_var('SHOW TABLES LIKE "' . $wpdb->prefix . AEC_CATEGORY_TABLE . '"') != $wpdb->prefix . AEC_CATEGORY_TABLE) {
				$sql = 'CREATE TABLE ' . $wpdb->prefix . AEC_CATEGORY_TABLE . ' (
						...
						VALUES 	(NULL, "Events", "517ed6", "FFFFFF");';
				dbDelta($sql);
			}
		...
		public function set_admin_alert($message) {
	    ...
	    public function localize_plugin($page) {
		...
		public function render_admin_menu() {
			if (function_exists('add_options_page')) {
				...
				$page = add_menu_page('Ajax Event Calendar',  __('Events', AEC_NAME), 'aec_add_events', AEC_FILE, array($this, 'render_admin_calendar'), 'dashicons-calendar', AEC_MENU_POSITION);
		...
		public function localized_variables() {
		...
		public function admin_calendar_variables() {
		...
		public function frontend_calendar_variables() {
			return array_merge($this->localized_variables(),
					array(
		...
		public function admin_category_variables() {
			return array_merge($this->localized_variables(),
				array(
		...
		public function calendar_styles() {
		...
		public function admin_category_styles() {
			wp_enqueue_style('categories');
			wp_enqueue_style('custom');
			if (is_rtl()) {
				wp_enqueue_style('custom_rtl');
		...
		public function admin_calendar_scripts() {
		...
		public function frontend_calendar_scripts() {
			if (is_admin()) {
				return;
			}
			wp_enqueue_script('jquery');
		...
		public function admin_social_scripts() {
			wp_enqueue_script('tweet');
			wp_enqueue_script('facebook');
		}

		public function admin_category_scripts() {
			wp_enqueue_script('jquery');
			wp_enqueue_script('growl');
		...
		public function render_aec_version() {
		...
		public function render_shortcode_calendar($atts) {
		...
		public function render_shortcode_eventlist($atts) {
			// shortcode defaults
			extract(shortcode_atts(array(
				'categories'	=> false,
				'excluded'		=> false,
		...
		public function render_calendar_events() {
		...
		public function render_eventlist_events($events, $whitelabel, $limit) {
		...
		public function render_category_filter($options, $categories=false, $excluded=false) {
		...
		public function render_calendar_options() {
			if (!current_user_can('aec_manage_calendar')) {
				wp_die(__('You do not have sufficient permissions to access this page.', AEC_NAME));
			}
		...
		public function process_events($events, $start, $end, $readonly) {
			if ($events) {
				$output	= array();
		...
		public function process_event($input, $readonly=false, $queue=false) {
			$output = array();
		...
		public function generate_repeating_event($event) {
		...
		public function get_next_date($date, $freq, $int) {
		...
		public function get_next_month($date, $n = 1) {
		...
		public function get_next_year($date, $n = 1) {
		...
		public function generate_event($input, $user_id) {
		...
		public function get_event_permissions($input, $user_id) {
			$permissions = new stdClass();
		...
		// OPTION CONTROLS
		public function add_panel($name, $content) {
			$before = "<div class='metabox-holder'>\n";
			$before .= "<div class='postbox' style='padding:30px 25px'>\n";
		...
		public function add_hidden_field($field, $value=false) {
			if ($value === false) {
				$aec_options 	= get_option('aec_options');
		...
		public function add_text_field($field, $label, $tip=false) {
			$aec_options = get_option('aec_options');
		...
		public function add_checkbox_field($field, $label, $tip=false) {
			$aec_options = get_option('aec_options');
			$checked = (isset($aec_options[$field]) && $aec_options[$field]) ? ' checked="checked" ' : ' ';
		...
		public function add_select_field($field, $label, $options, $values=false) {
			$aec_options = get_option('aec_options');
		...
		public function add_wrap($content, $before='', $after='') {
		...
		public function delete_events_by_user() {
		...
		public function update_filter_label() {
		...
		public function render_category($input) {
			$output = array(
				'id'	 	=> $input->id,
		...
		public function reassign_category() {
			if (!isset($_POST['id'])) {
				return;
			}
		...
		public function confirm_delete_category() {
			if (!isset($_POST['id'])) {
				return;
			}
		...
		// DATABASE QUERIES
		public function db_query_monthly_activity() {
			global $wpdb;
		...
		public function db_query_event($id) {
			global $wpdb;
		...
		public function db_query_events($start, $end, $category_id, $excluded, $limit=false) {
			global $wpdb;
		...
		public function db_query_events_by_user($user_id) {
			global $wpdb;
		...
		public function db_query_events_by_category($id) {
			global $wpdb;
		...
		public function db_insert_event($input, $render) {
			global $wpdb;
		...
		public function db_update_event($input) {
			global $wpdb;
		...
		public function db_delete_event() {
			global $wpdb;
		...
		public function db_delete_events_by_user($id) {
			global $wpdb;
			$result = $wpdb->query($wpdb->prepare('DELETE FROM ' . $wpdb->prefix . AEC_EVENT_TABLE . ' WHERE user_id = %d;', $id));
			return $this->return_result($result);
		}

		public function db_query_categories($category_id=false, $excluded=false) {
			global $wpdb;
			$excluded = ($excluded) ? 'NOT IN' : 'IN';
		...
		public function db_insert_category($input) {
			global $wpdb;
		...
		public function db_update_category($input) {
			global $wpdb;
		...
		public function db_reassign_category($id) {
			global $wpdb;
			$result = $wpdb->update($wpdb->prefix . AEC_EVENT_TABLE,
								array('category_id'	=> 1),
		...
		public function db_delete_category($id) {
			global $wpdb;
		...
		public function cleanse_output($output) {
			if (is_array($output)) {
		...
		public function cleanse_data_input($input) {
			return trim($input);
		}

		public function cleanse_event_input($input) {
		...
		public function cleanse_category_input($input) {
			$clean 			= $this->convert_array_to_object($this->parse_input($input));
		...
		public function cleanse_shortcode_input($input) {
			$input = explode(',', $input);
		...
		public function convert_array_to_object($array = array()) {
			$return = new stdClass();
		...
		public function return_current_user_id() {
			global $current_user;
			get_currentuserinfo();
			return $current_user->ID;
		}

		public function return_auth($readonly = false) {
			if ($readonly) {
				return "-1";
			}
		...
		public function render_i18n_data($data) {
			return htmlentities(stripslashes($data), ENT_COMPAT, 'UTF-8');
		}

		public function render_json($output) {
		...
		public function overwrite_option($key, $value) {
			$options = get_option('aec_options');
			$options[$key] = $value;
		...
		public function insert_option($key, $value) {
			$options = get_option('aec_options');
		...
		public function decommission_options($keys) {
			$options = get_option('aec_options');
			foreach ($keys as $key) {
				if (array_key_exists($key, $options)) {
					unset($options[$key]);
		...
		public function convert_date($date, $from, $to=false) {
		...
		public function parse_date_format($format) {
		...
		public function parse_time_format($format) {
		...
		public function array_compare_order($a, $b) {
		...
		public function add_events_column_data($empty='', $column_name, $user_id) {
		...
		public function set_option_page_capability($capability) {
			return 'aec_manage_calendar';
		}

		public function admin_options_initialize() {
